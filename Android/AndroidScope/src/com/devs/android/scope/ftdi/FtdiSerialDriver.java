...
package com.devs.android.scope.ftdi;
...
public class FtdiSerialDriver extends CommonUsbSerialDriver {
    ...
    public void setDTR(boolean value) throws IOException {
		...
		final int FTDI_DEVICE_OUT_REQTYPE = UsbConstants.USB_TYPE_VENDOR | USB_RECIP_DEVICE | UsbConstants.USB_DIR_OUT;

		/** The Constant SIO_SET_MODEM_CTRL_REQUEST. */
		final int SIO_SET_MODEM_CTRL_REQUEST = 0x01;
		...
		final int SIO_SET_DTR_MASK = 0x1;

		/** The Constant SIO_SET_DTR_HIGH. */
		final int SIO_SET_DTR_HIGH = (1 | (SIO_SET_DTR_MASK << 8));

		/** The Constant SIO_SET_DTR_LOW. */
		final int SIO_SET_DTR_LOW = (0 | (SIO_SET_DTR_MASK << 8));

		if (value) {

		}

		int dtr = value ? SIO_SET_DTR_HIGH : SIO_SET_DTR_LOW;

		int r;
		if ((r = mConnection.controlTransfer(FTDI_DEVICE_OUT_REQTYPE, SIO_SET_MODEM_CTRL_REQUEST, dtr, mInterface, null, 0, USB_WRITE_TIMEOUT_MILLIS)) != 0) {
			throw new IOException("USB controlTransfer operation failed. controlTransfer return value is:" + Integer.toString(r));
